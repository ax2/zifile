...
核心采用纯 Rust 或 Rust 原生绑定的可替换后端组合：ZIP 使用 `zip`，7z 使用 `sevenz-rust2`，TAR 使用 `tar`，gzip 使用 `flate2`，Zstandard 使用 `zstd`，XZ 使用静态链接的 `xz2`，Bzip2 使用 `bzip2`，LZ4 使用 `lz4_flex`，Brotli 使用 `brotli`。
...
- RAR 创建不进入范围；RAR 读取在许可证、维护状态和恶意语料评审前保持禁用。
