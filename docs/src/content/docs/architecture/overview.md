...
两套 UI 还共享一个容量为 32（含运行中任务）的内存 FIFO 调度器。打开、重载、校验、解压、创建和更新在提交时快照请求并依次启动独立 Worker；更新操作在同目录临时区中完成解包、合并和重建，成功后才原子替换原归档。便携桌面 EXE 会以内部 Worker 模式重新启动自身，MSIX/构建证据仍保留独立的 `zifile-worker.exe` 兼容入口。任务完成或取消后才启动下一项。完成消息携带单调 ID，陈旧/重复完成事件不会推进队列。用户可以清空等待项而不误取消当前 Worker；待处理请求随清空或退出直接释放，不持久化其中的路径或密码。

### Worker

桌面端所有列出、校验、解压和创建请求均通过版本化 JSON Lines 协议发送给 Worker。独立便携 EXE 通过 `--zifile-worker` 参数在同一个文件内进入 Worker 模式；独立的 `zifile-worker.exe` 仍支持 MSIX 和开发环境兼容运行。归档条目逐条返回，避免十万条目形成单个巨型 IPC 消息。
