...
两套 UI 还共享一个容量为 32（含运行中任务）的内存 FIFO 调度器。打开、重载、校验、解压和创建在提交时快照请求并依次启动独立 Worker；便携桌面 EXE 会以内部 Worker 模式重新启动自身，MSIX/构建证据仍保留独立的 `zifile-worker.exe` 兼容入口。任务完成或取消后才启动下一项。完成消息携带单调 ID，陈旧/重复完成事件不会推进队列。用户可以清空等待项而不误取消当前 Worker；待处理请求随清空或退出直接释放，不持久化其中的路径或密码。
