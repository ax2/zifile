...
创建 ZIP、7z 和 TAR 组合（包括 TAR + LZMA 与 TAR + LZ4）时可以选择多个文件或文件夹。单流 gzip、Zstandard、XZ、LZMA、Bzip2、LZ4 和 Brotli 必须恰好选择一个现有文件；若要压缩目录或多个项目，请选择对应的 TAR 组合格式。桌面端会在打开保存对话框前检查这一要求。
