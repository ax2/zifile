...
ZIP、7z、CAB 和 TAR 组合支持多个文件与文件夹；CAB 使用固定 MSZIP 压缩，不能保存空目录。gzip、Zstandard、XZ、LZMA、Bzip2、LZ4 和 Brotli 是单文件流，必须恰好选择一个现有文件；需要压缩目录时，请改用对应 TAR 组合。RAR 只支持读取，不支持创建。
