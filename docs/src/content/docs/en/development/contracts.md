...
The 1.0 candidate keeps these commands: `formats`, `detect`, `list`, `test`, `extract`, `create`, `update`, and `rename`. Creation format values are `zip`, `seven-zip`, `tar`, `tar-gzip`, `tar-zstd`, `tar-xz`, `tar-lzma`, `tar-bzip2`, `tar-lz4`, `gzip`, `zstandard`, `xz`, `lzma`, `bzip2`, `lz4`, and `brotli`. Conflict values are `overwrite`, `skip`, `rename`, and `error`.
