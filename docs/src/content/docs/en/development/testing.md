...
`tests/helpers/msix-repair` is a C# test-only console helper; the product remains Rust-first. CI compiles it against a locked Windows App SDK 1.8 dependency and runs its non-mutating `--probe`. The API query runs in an isolated child that does not spawn descendants; the parent terminates that child directly after 15 seconds, while the workflow adds a two-minute outer bound. A Runner that does not return therefore records an incomplete/unsupported probe instead of hanging or claiming Repair passed. When Repair is supported, the trusted lifecycle writes a random package LocalState sentinel, requires `RepairPackageAsync` to preserve it, then requires `Reset-AppxPackage` to remove it. Unsupported systems record `unsupported` explicitly.
