...
Integrity testing uses `TestOptions` to provide common entry/byte progress and cooperative cancellation for ZIP, 7z, RAR, CAB, TAR compositions, and all six compression streams. The Worker emits bounded updates every 100 ms and a final snapshot after the operation returns, so small archives do not expose only an initial 0% value. A pre-cancel regression requires `Cancelled` before parsing begins. Fine-grained list progress remains separate work.
