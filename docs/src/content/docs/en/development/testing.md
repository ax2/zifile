...
Compression-level contract tests cover every encoder's valid range, boundary clamping, and fixed-level formats. A 7z integration regression creates level-0 and level-9 archives, reads the public LZMA2 coder properties, and requires them to differ. This proves from archive metadata that the UI/CLI setting affects the encoder instead of merely traversing the Worker protocol.
