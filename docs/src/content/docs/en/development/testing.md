...
`tests/smoke/packaging-policy.ps1` dynamically parses the current fifteen release, corpus, and repository-policy PowerShell scripts. It rejects missing formal/cloud inputs, unsupported providers, development identities, unsigned OID publishers, invalid signed artifacts, and incomplete 1.0 readiness; it accepts the valid input and fully evidenced 11/11 fixtures. It also requires post-signing audit, signed-only publishing, least privilege, signing timeout/concurrency, the rotation/emergency-stop/revocation runbook, and version, release-note, contributor, security, and readiness gates in CI. Policy smoke cannot replace a real cloud-HSM signature or x64/ARM64 package-content audit.
