...
`tests/helpers/msix-repair` is a C# test-only console helper; the product remains Rust-first. CI compiles it against a locked Windows App SDK 1.8 dependency and runs its non-mutating `--probe`. When Repair is supported, the trusted lifecycle writes a random package LocalState sentinel, requires `RepairPackageAsync` to preserve it, then requires `Reset-AppxPackage` to remove it. Unsupported systems record `unsupported` explicitly.
