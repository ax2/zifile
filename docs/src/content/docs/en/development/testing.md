...
`tests/smoke/packaging-policy.ps1` dynamically parses the current twelve release, corpus, and repository-policy PowerShell scripts. It rejects missing publishing inputs and an incomplete 1.0 manifest, accepts a fully evidenced 11/11 readiness fixture, requires package audits and uploaded audit JSON, and confirms that version, release-notes, contributor, security, and release-readiness gates are wired into CI. This policy smoke does not replace the real x64/ARM64 package-content audit performed by `Build-Package.ps1`.
