...
`tests/smoke/packaging-policy.ps1` dynamically parses the current ten release, corpus, and repository-policy PowerShell scripts. It exercises publishing-input rejection and acceptance, requires package audits and uploaded audit JSON, and confirms that version, release-notes, and contributor-documentation gates are wired into CI. This policy smoke does not replace the real x64/ARM64 package-content audit performed by `Build-Package.ps1`.
