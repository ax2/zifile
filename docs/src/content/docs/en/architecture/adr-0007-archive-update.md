...
Users need to add files or folders from the archive browser. The current ZIP and 7z provider boundaries do not expose a reusable in-place append abstraction, and TAR compositions must rewrite their outer stream. Overwriting the original directly could leave an unusable archive after an encoder error, cancellation, or disk failure.
...
- The core fully lists and safely extracts the original into a sibling staging workspace, then merges additions by source root. Colliding regular files are replaced; file/directory type collisions, links, and reparse points are rejected.
