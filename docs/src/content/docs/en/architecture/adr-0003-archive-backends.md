...
ZiFile uses replaceable pure-Rust or Rust-native backends: `zip` for ZIP, `sevenz-rust2` for 7z, `rars` for read-only RAR, `cab` for CAB creation and reading, `tar`, `flate2` for gzip, `zstd`, statically linked `xz2` for XZ, `lzma-rust2` for standalone LZMA, `bzip2`, `lz4_flex`, and `brotli`.
