...
ZiFile uses replaceable pure-Rust or Rust-native backends: `zip` for ZIP, `sevenz-rust2` for 7z, `tar`, `flate2` for gzip, `zstd`, statically linked `xz2`, `bzip2`, `lz4_flex`, and `brotli`.
...
- RAR creation is out of scope; read support stays disabled pending licensing, maintenance, and hostile-input review.
