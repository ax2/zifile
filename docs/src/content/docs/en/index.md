...
The project is currently in **Stage 4 — public release and 1.0 readiness**. ZIP, 7z, RAR 5, TAR compositions, Windows CAB creation with fixed MSZIP, and the main single-stream formats support real creation, browsing, integrity testing, and safe extraction; RAR 1.3–7 remains a beta provider. The desktop UI and CLI share the same Rust core, while x64/ARM64 builds, package audits, reproducibility checks, and release rehearsals are automated. Trusted signing, real foreground validation, physical ARM64, WACK, Partner Center, Store, and WinGet remain 1.0 external gates.
