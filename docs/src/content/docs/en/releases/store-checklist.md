...
- The desktop EXE and Explorer shell entry share one reviewed multi-resolution Win32 ICO with 16, 24, 32, 48, and 256 pixel 32-bit PNG frames. Machine gates verify generation, directory structure, dimensions, and the unpacked package hash; package audit also reads the desktop EXE's `GROUP_ICON`/`ICON` resources frame by frame so a merely adjacent, unembedded ICO cannot pass.
