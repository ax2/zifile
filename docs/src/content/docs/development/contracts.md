...
1.0 候选保留以下命令：`formats`、`detect`、`list`、`test`、`extract`、`create`、`update`。创建格式值为 `zip`、`seven-zip`、`tar`、`tar-gzip`、`tar-zstd`、`tar-xz`、`tar-lzma`、`tar-bzip2`、`gzip`、`zstandard`、`xz`、`lzma`、`bzip2`、`lz4` 和 `brotli`；冲突策略为 `overwrite`、`skip`、`rename` 和 `error`。
...
Worker JSON Lines IPC 使用独立的 `PROTOCOL_VERSION`；当前版本为 `2`，因为新增更新归档请求改变了请求枚举。旧版客户端/Worker 必须明确拒绝，不能根据字段猜测版本；同一协议版本内缺失的可选归档字段仍按默认值兼容。
