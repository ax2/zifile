...
1.0 候选保留以下命令：`formats`、`detect`、`list`、`test`、`extract`、`create`。创建格式值为 `zip`、`seven-zip`、`tar`、`tar-gzip`、`tar-zstd`、`tar-xz`、`tar-lzma`、`tar-bzip2`、`gzip`、`zstandard`、`xz`、`lzma`、`bzip2`、`lz4` 和 `brotli`；冲突策略为 `overwrite`、`skip`、`rename` 和 `error`。
