...
核心往返测试还覆盖“更新现有归档”：ZIP、7z 和全部 TAR 组合都会验证同名普通文件替换、嵌套新文件加入以及更新后重新解包；gzip 等单文件流、RAR 和 CAB 则验证明确的只读错误。更新使用同目录临时工作区，失败或取消不得替换原归档。
...
打开失败回归会区分密码/加密诊断与损坏、未知格式及普通 I/O 错误：可能需要密码时显示密码输入和专用“解锁”操作；其余失败保留待打开路径并显示普通“重新加载”，便于文件刚写完或临时 I/O 故障后的单次重试。共享组合门禁要求仅在空闲、存在待打开路径且不需要密码时启用普通重试，加载中保持禁用，初始空状态不显示；两套 UI 还会在应用结果前拒绝陈旧 Worker 完成事件，避免旧任务覆盖当前归档或忙碌状态。
...
创建加密压缩文件时，两套 UI 还会在打开保存对话框前要求两次密码完全一致。共享纯函数回归覆盖双空值、精确匹配、大小写差异和任一字段为空；不匹配门禁始终生效，但内联提示会等用户开始操作确认字段后再出现。可访问候选提供独立标签的确认字段、`aria-invalid`、警报说明，并让一个显示密码复选框同时控制两个字段。创建请求被接受后会释放两个瞬时值及交互状态，切换到不支持加密的格式时也会一并清除。
...
解压操作区回归要求两套 UI 在重命名、覆盖、跳过和报错选项上方提供可见的双语“文件冲突策略”字段名。默认 Iced 使用同一列中的文本标签，可访问 Dioxus 使用原生 `label` 包裹下拉框；不能只依赖选项内容或仅辅助技术可见的名称说明字段职责。

默认 Iced 的归档密码和搜索字段还必须保留持久可见的双语标签，不能只使用输入框 placeholder；用户输入内容后，字段用途仍应可见。回归同时锁定搜索框 ID、清除动作以及原有密码遮蔽接线，确保布局调整不破坏 `Ctrl+F` 聚焦、搜索清除或密码可见性行为。可访问 Dioxus 已使用原生标签，继续保持该语义基线。

归档标题的共享大小摘要会同时显示展开大小、压缩后大小和相对比例。单测覆盖正常缩小、压缩后反而增大及空归档：缩小时使用“缩小/smaller”，增大时使用“增大/larger”，不能显示负节省率；展开大小为零时省略没有意义的百分比。两套 UI 的正常和忙碌归档标题都必须传入真实 `total_size` 与 `compressed_size`。
