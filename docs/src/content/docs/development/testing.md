...
密码可见性回归覆盖两套 UI 的默认遮蔽、归档与创建字段接线，以及密码释放时同步恢复遮蔽。可访问候选还锁定三个唯一字段 ID、原生复选框标签、动态 `password`/`text` 类型和对应 `aria-controls`；这些源码与单元测试证据不替代真实屏幕阅读器或前台视觉复验。

创建加密压缩文件时，两套 UI 还会在打开保存对话框前要求两次密码完全一致。共享纯函数回归覆盖双空值、精确匹配、大小写差异和任一字段为空；可访问候选提供独立标签的确认字段、`aria-invalid`、警报说明，并让一个显示密码复选框同时控制两个字段。创建请求被接受后会释放两个瞬时值，切换到不支持加密的格式时也会一并清除。

加密文件列表的密码输入支持 `Enter` 提交。默认 UI 通过 Iced 输入提交事件复用现有重新加载路径；可访问候选的纯函数回归要求操作可用、非 IME 组合输入且没有 Ctrl/Alt/Shift/Meta 修饰键，锁定键不影响提交，忙碌状态不会重复排队。
...
根级键盘处理器只在存在活动取消令牌时接管并阻止 `Escape` 默认行为；空闲状态返回未处理，使原生控件保留关闭或退出交互。取消函数本身也在令牌缺失时保持无操作，不写入虚假的“正在取消”状态。

快捷键匹配会忽略 Caps Lock、Num Lock 等锁定状态，但要求其他修饰键与公开契约精确一致；因此 `Ctrl+Shift+N`、`Alt+F1` 和 `Ctrl+Shift+A` 不会被降级解释为 `Ctrl+N`、`F1` 或 `Ctrl+A`。回归覆盖正向组合、锁定键兼容以及 Shift/Alt 负向组合。

默认 Iced 桌面的 `Ctrl+O`、`Ctrl+N`、归档区域 `Ctrl+A`、`F1` 和 `Escape` 同样遵循功能修饰键精确匹配。Iced 修饰键类型只包含 Shift、Control、Alt 与 Logo，因此无修饰命令要求 `NONE`，Control 命令要求精确等于 `CTRL`；纯事件映射回归同时覆盖正向和额外修饰键组合。

可访问候选在首页、归档、创建和关于页之间切换时，会把焦点移到稳定的主内容区域，并用当前页面标题标记该区域。该 effect 以页面身份为门禁，因此进度轮询、队列更新、筛选、选择等同页重绘不会抢走焦点。源码与单元回归锁定页面到标题的映射及焦点触发接线；真实 Narrator 与前台键盘验证仍然必须补充。
