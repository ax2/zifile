...
当前没有等待发布的功能增量。RAR 5 创建和固定 MSZIP CAB 创建已随 v0.1.15 发布；TAR + LZ4 已随 v0.1.13 发布，v0.1.14 已发布归档操作栏专业化分组。
...
- `v0.1.0-alpha.1` 已作为公开 prerelease 发布；`v0.1.0`、`v0.1.2`、`v0.1.3`、`v0.1.4`、`v0.1.5`、`v0.1.6`、`v0.1.7`、`v0.1.8`、`v0.1.9`、`v0.1.10`、`v0.1.11`、`v0.1.12`、`v0.1.13`、`v0.1.14` 和 `v0.1.15` 均通过匹配 tag 自动生成公开 GitHub Release。
...
稳定 1.0 尚未发布；`v0.1.15` 是当前面向 GitHub 的公开可用版本，但正式 Store、WinGet 和可信签名门禁仍未完成。Stage 4 保持进行中。

## 2026-09-01 v0.1.15 RAR 5/CAB 创建与公开版本结果

- RAR 5 创建支持等级 0–5、密码加密文件头、进度、取消和原子输出；RAR 分卷、恢复记录、更新和重命名限制已明确。Windows CAB 创建使用固定 MSZIP，并与核心、CLI、两套桌面 UI、列表/校验/选择性解压往返测试和双语文档贯通；空目录、密码、更新和重命名限制已明确。
- PR [#106](https://github.com/ax2/zifile/pull/106) 合并提交为 `13f1d0e25c3a546ebe2f88b172feb2479de05d67`，tag `v0.1.15` 精确指向该提交。
- [Release workflow 33502996127](https://github.com/ax2/zifile/actions/runs/33502996127) 成功完成 SBOM、x64/ARM64 测试和构建、独立 EXE 冒烟/审计、all-in-one bundle、资产审计和公开发布。
- [v0.1.15 Release](https://github.com/ax2/zifile/releases/tag/v0.1.15) 于 `2026-09-01T12:05:59Z` 发布，非草稿、非预发布，严格只公开 `ZiFile-0.1.15.0-windows.msixbundle`、`zifile-windows-x64.exe`、`zifile-windows-arm64.exe` 和 `SHA256SUMS.txt`；没有 DLL、JSON、YAML 或 ZIP。
...
- 两套桌面归档页标题栏新增“在资源管理器中显示”操作，调用 Windows Explorer 选中当前归档；启动失败进入错误状态，该操作本身不新增设置或日志记录。成功打开归档的路径后来按“最近打开”政策保存。
...
- 本地 WinGet 1.29.290 的官方 `validate` 接受 schema 1.12 四文件候选；31 个扩展名同步、哈希篡改拒绝和完整 packaging policy 均通过。随后补充的 MSIX 内嵌身份预检会拒绝 `ZiCode.ZiFile.Dev` 开发包，避免把未签名开发身份提交到社区仓库；社区仓库接受与签名门禁仍未完成。
...
## 2026-08-31 — 一键解压到同名文件夹

- 两套桌面 UI 的归档操作区新增“解压到同名文件夹”，无需打开文件夹选择器即可把全部内容解压到归档旁的匹配目录。
- 该入口复用资源管理器 `--extract-here` 的复合扩展名规则：`示例.zip` 对应 `示例/`，`backup.tar.gz` 对应 `backup/`；提交仍经过当前冲突策略、容量受限 FIFO 队列、隔离 Worker 和完成输出定位。
- 默认 UI 单测锁定实际 Worker 请求的归档路径、目标路径、全量范围和会话密码传递；可访问候选由界面接线回归锁定。真实前台按钮操作仍待 Computer Use 宿主管道恢复后复验。

## 2026-08-31 — 0.1.6 发布准备
...
- 用户可在主页一键重新打开、逐项移除或清空历史；忙碌期间入口禁用，移除记录不删除文件、不关闭当前归档，也不触碰操作队列。
- 设置继续使用临时文件 flush/sync 后原子替换；路径以有界十六进制字段编码，避免换行和等号注入配置结构。设置单测覆盖 Unicode/分隔符往返、畸形输入、容量、排序和 Windows 路径去重，两套 UI 目标均完成编译与单测。
- 隐私一致性审计同步修正 README、双语隐私政策和桌面文档：明确披露最多 8 条本地路径、保留周期、逐项/全部删除控制，以及十六进制字段编码不是加密。

## 2026-08-31 — 归档批量选择操作

- 两套归档页新增明确的“全选”“全不选”“反选”，不再要求用户从动态复选框状态推断清空动作；批量作用域统一为整个归档的普通文件，不受当前文件夹、搜索或分页影响。
- 新增精确 `Ctrl+I` 反选，保留 `Ctrl+A` 全选；Dioxus 控件公开 `aria-keyshortcuts="Control+I"`，批量动作通过现有状态区域报告最新数量，并保持 IME 组合输入保护。
- 共享线性选择函数忽略目录并原地更新集合；单测覆盖混合文件/目录集合，Criterion 新增 100,000 文件半选反转基准，两套 UI 的快捷键和可见帮助由回归锁定。

## 2026-08-31 — 独立 EXE 完整往返门禁

- 确认桌面 EXE 通过自身路径加 `--zifile-worker` 启动受约束的隔离子进程，公开的单文件下载不依赖同目录 `zifile-worker.exe`。
- x64 Release 冒烟从“读取外部创建的 ZIP”升级为仅使用这一个 EXE 完成创建 ZIP、列出项目、解压，并逐字节核对源文件；临时目录明确拒绝出现旁置 Worker。
- ARM64 在 x64 runner 上继续执行 PE 架构和无旁置 Worker 审计，完整运行仍保留给物理 ARM64 门禁，不以交叉架构静态检查冒充实机证据。

## 2026-08-31 — 0.1.7 发布准备
