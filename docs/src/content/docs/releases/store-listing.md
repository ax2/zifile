...
短描述：面向 Windows 10 和 Windows 11 的现代开源压缩工具。浏览并安全解压 RAR、ZIP、7z 和 TAR，创建开放格式，支持加密、拖放和命令行。

## 完整描述

ZiFile 是一款面向 Windows 的现代开源压缩与归档工具，提供清晰的操作流程和谨慎的安全默认值。

你可以浏览压缩包内容，只解压选中的项目，或把文件和文件夹创建为 ZIP、7z、TAR 以及常用的压缩流。RAR 1.3 至 RAR 7 支持浏览、校验和解压，但不能创建。ZIP 和 7z 支持 AES 加密；密码不会写入日志或设置。
