...
你可以浏览压缩包内容，只解压选中的项目，或把文件和文件夹创建为 ZIP、7z、TAR、固定 MSZIP 的 Windows CAB 以及常用的压缩流。RAR 1.3 至 RAR 7 支持浏览、校验和解压但不能创建；CAB 不支持密码、更新或重命名，也不能保存空目录。ZIP 和 7z 支持 AES 加密；密码不会写入日志或设置。
