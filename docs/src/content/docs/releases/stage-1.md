...
- 实现 ZIP/ZIP64/AES、7z/AES、TAR、tar.gz、tar.zst、tar.xz、tar.lzma、tar.bz2，以及六种单流格式。
...
- 打开文件对话框的共享扩展名清单补齐 `tar.gz`、`tar.zst`、`tar.xz` 和 `tar.bz2`，使已支持的 TAR 复合格式可以从常见的复合后缀直接发现。
- 打包策略 smoke 同时显式锁定这四个复合后缀，防止核心打开清单和发布检查再次漂移。
...
打开大型归档此前虽在隔离 Worker 中运行且可由桌面端强制回收，但核心 List API 没有统一进度与协作取消。新增兼容旧入口的 `ListOptions` 和 `list_archive_with_options`；ZIP、7z、RAR 1.3–7、CAB、六种 TAR 组合与六种单压缩流均在扫描边界检查取消并推进进度。已知条目总数的格式显示确定进度，TAR/CAB 等总数尚未知时显示双语“正在扫描”，完成后再提交一致的最终总量；单压缩流同时报告实际解码字节。
...
ZIP 创建现在写入源文件和目录的修改时间。ZIP、7z、六种 TAR 组合、RAR 与 CAB 的安全解压都在文件原子提交后恢复可用修改时间；目录时间延迟到所有子项完成，再按路径从深到浅恢复，避免后续写入覆盖父目录元数据。往返测试覆盖两个文件和嵌套目录，固定 RAR 5 与 CAB 夹具则证明只读 Provider 能恢复独立创建的时间字段。
...
进行中。已验证本地 x64 开发 MSIX、候选运行目录和远程 x64/ARM64 默认/候选非发布产物链；当前提交仍是 Alpha 开发检查点，不是可上架版本。公开 Alpha prerelease 的发布记录见 Stage 3；最新演练证据保存在 [GitHub Actions run 32685678567](https://github.com/ax2/zifile/actions/runs/32685678567)。
...
## 2026-08-29 — Explorer 文件夹背景命令

### 修改

- 将“使用 ZiFile 创建压缩文件”注册到 Windows 11 Explorer 的 `Directory\Background` 项目类型。右键文件夹空白区域时，Explorer 传入当前文件夹，现有 Rust COM 命令通过相同的 `--create` 协议打开创建页；归档工作仍由桌面和隔离 Worker 完成。

### 验证

- MSIX manifest 审计与打包策略门禁均要求 `*`、`Directory` 和 `Directory\Background` 三种创建上下文。当前开发包仍未签名，真实 Explorer 激活与生命周期证据继续保留在可信安装门禁中。
- 早期真实前台队列诊断记录：默认 Iced 窗口取得了前台句柄，但没有暴露 UI Automation `Document` 文本；Dioxus 候选曾有一次未获宿主前台资格。脚本在这些回合的业务断言前安全停止并清理进程；该历史记录不覆盖后续成功回合。
- Worker 增加仅测试用的 `ZIFILE_TEST_WORKER_DELAY_MS`，上限 10 秒且延迟期间继续监听取消；队列脚本默认使用 10 秒启动窗口，并在每次排队前重新获取重绘后的 UIA 按钮，避免磁盘速度与失效元素引用造成偶发结果。
- 在当前宿主以 `zifile-desktop-accessible.exe` 完成真实前台 100,000 条目、三次校验回合：`foreground_window_verified=true`、活动项取消、下一项启动、等待项清空、Worker 回收和零部分输出均通过。默认 Iced 仍因没有可认证的 UIA `Document` 语义树不能纳入同一门禁，Stage 1 队列项保持开放。
- 为消除大归档队列切换时的瞬时完整表格重建，两套桌面 UI 现在只有在没有下一项时才清除 `busy`；有下一项时直接交接到下一个 Worker。默认 Iced 与可访问候选均增加源码回归断言，防止重新引入该状态窗口。
- 细节修复：Explorer 解压命令在允许慢查询时改用核心签名检测与能力注册表；有效归档改名后仍可显示命令，无效文件不会仅凭伪造 `.zip` 后缀显示，同时仍不会新增 `.epub` 默认文件关联。
- 生命周期修复：Shell DLL 为 COM 对象和 `LockServer` 建立原子计数，只有两者均归零时 `DllCanUnloadNow` 才允许卸载，并增加回归单测。
- 背景菜单稳健性修复：空 `IShellItemArray` 时通过 `IObjectWithSite` 查询 Explorer 当前文件夹，成功后作为单一创建来源；无法取得站点时不猜测路径。
- Shell 创建来源去重修复：命令在启动桌面前按 Windows 路径身份合并大小写或斜杠写法相同的来源，避免重复根目录和无谓的命令行长度；解压命令仍要求单个 Explorer 项目。
- 单流输出名修复：`.lzma` 和 `.bz` 这两个历史别名现在会正确去除输入后缀，解压结果保留原始文件名主体，不再错误追加 `.out`；新增核心回归单测覆盖规范后缀、别名、大小写和未知后缀兜底。
- LZMA 兼容性修复：`.lzma` 没有统一 magic 时由后缀进入 Xz 格式族，再交给纯 Rust `lzma-rust2` 的 LZMA-alone 解码器；现有安全限制会转换为解码器内存上限，固定 37 字节语料已完成列表与解压互操作回归。
- TAR + LZMA 兼容性修复：`.tar.lzma` 现在拥有独立格式契约，支持纯 Rust LZMA-alone 创建与解码，并完成列表/校验/解压/创建 round trip，不再误判为单文件流。
- UI 一致性修复：创建格式顺序集中到核心 `ArchiveFormat::CREATABLE` 注册表，Iced 与 Dioxus 候选共用同一份 14 项创建菜单，并由 CLI 契约冒烟锁定，避免新增 Provider 时两套 UI 发生漂移。
- Explorer 细节修复：Shell 解压命令复用核心格式能力与后缀识别，并要求项目实际是文件；名为 `folder.zip` 的普通目录不再错误显示解压菜单。
- 创建流程细节修复：共享 UI 预检现在会在保存对话框前发现已消失的来源，并在两套桌面界面显示双语恢复提示；Worker 仍保留最终竞态校验。
- 拖放一致性修复：默认 Iced 与 Dioxus 候选共用签名优先的归档分类，支持改名归档，并保留无通用 magic 格式与探测失败时的扩展名回退。
- 拖放线程修复：Iced 与 Dioxus 都把文件头探测移出 UI 事件线程，慢速或远程路径不会阻塞窗口。
- 来源去重修复：两套桌面界面共用 Windows 路径身份比较，文件选择器、Explorer 多选和拖放使用不同大小写或斜杠写法时，不会重复提交同一个来源。
- Windows 路径细节修复：来源身份比较改用 Unicode 小写归一化，非 ASCII 文件名的大小写变体也不会被重复加入。
- 契约收敛修复：匹配目录命名现在只使用核心实际支持的 TAR 组合，不再把 `.tar.lz4`、`.tar.br` 或 `.tar.bz` 误当成可解压格式。
- 注册表一致性修复：核心格式检测与 Explorer 同名目录命名共用复合 TAR 后缀注册表，新增别名时不会只更新其中一条路径。
- 取消语义修复：创建和解压在创建父目录/目标目录前再次检查取消信号，已取消操作不会留下可避免的空目录。
- 输出安全修复：创建和解压完成后直接用临时文件原子替换旧目标，不再先删除旧文件；最终替换失败时可保留原有内容。

## 2026-08-29 — 互操作与 CLI 契约门禁

### 修改

- Windows 参考工具脚本新增 TAR + LZMA-alone 双向场景：系统 `tar.exe --lzma` 创建的归档由 ZiFile 校验并解压，ZiFile 创建的归档由 `tar.exe` 解压；脚本同时生成不含用户数据的 8 场景 JSON 证据。
- 新增 CLI 契约冒烟，锁定 6 个公开命令、14 个创建格式、16 行 `formats` 能力矩阵、双语契约文档及运行时/语法错误码，并接入 Windows CI 与发布策略检查。
- 修正公开契约文档遗漏 `tar-lzma` 的格式列表，并同步 CHANGELOG 与测试策略说明。

### 验证

- 本机 Windows 参考工具互操作 8/8 通过；CLI 契约冒烟、Foundation smoke、14 个核心单测、34 个归档集成测试、严格 Clippy、策略门禁和 63 页 Astro 构建均通过。
- 当前 `tar.exe` 反向解压 TAR+LZMA 时对非 ASCII TAR 文件名受 Windows 代码页影响，因此该方向严格核对 ASCII 文件内容；参考工具创建到 ZiFile 的方向仍核对 Unicode 路径，其他 ZIP、tar.gz、7z 方向继续覆盖 Unicode。
