...
- 本地 x64 候选 `0.1.0.3` MSIX 与完整可运行目录构建成功；包内候选桌面 EXE 与 Release 构建 SHA-256 一致，可运行目录含桌面、CLI、Worker、MIT 许可证和 README，不含 ZIP。完整目录启动后 UI Automation 再次识别 `main-content` 与 live status。ARM64 候选仍等待云端工作流复验。
...
- 完整键盘遍历、`Ctrl+A`、屏幕阅读器、高对比度、中文 IME 和每显示器 DPI 验证；十万条目的峰值内存与交互延迟仍需正式性能基线。
- Iced 当前没有可用于认证的完整 Windows UI Automation/Narrator 语义树；Dioxus 候选已证明 UI Automation 语义树、Worker 功能路径、核心快捷键与本地 x64 打包，但默认替换仍受 Narrator、Accessibility Insights、高对比度、IME、DPI、真实拖放和 ARM64 候选运行门禁约束。
